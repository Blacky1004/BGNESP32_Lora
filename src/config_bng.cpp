...
fileversions_info_t fileVersion;
	...
	if (jsonFile)
	{
		...
		if (doc.containsKey("clients"))
		{
			...
			for (JsonObject o : jarr)
			{
				...
				t.id = o["id"];
				strcpy(t.url, o["url"]);
				strcpy(t.name, o["name"]);
				urlList.push_back(t);
				i++;
			}
			ESP_LOGI(TAG, "Es wurden %d Daten aus der RestURL-Liste geladen.", i);
		}
		else
		{
			ESP_LOGI(TAG, "Keine REST-Url daten vorhanden.");
		}
		jsonFile.close();
	}
}
...
void loadFileVersionsConfig(void)
{
	...
	File jsonFile = SPIFFS.open("/fsversion.json", "r");
	if (jsonFile)
	{
		deserializeJson(doc, jsonFile);
		strcpy(fileVersion.max_version, doc["max_version"]);
		strcpy(fileVersion.min_version, doc["min_version"]);
		strcpy(fileVersion.version, doc["version"]);
		jsonFile.close();
	}
}
