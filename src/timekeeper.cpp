...
#ifdef HAS_IF482
    HardwareSerial IF482(2) // Den 2. UART benutzen, da der erste bereits vom GPS Tracker verwendet wird.
    
    #if (HAS_SDS011)
    #error Der IF482 kann nicht zusammen mit dem SDS011 verwendet werden! Beide beutzen den UART#2
    #endif
#endif
...
void GetMyTime(void) {
  tm local;
  getLocalTime(&local);
  snprintf(systemCfg.last_payload,28,"%d-%02u-%02uT%02u:%02u:%02u", local.tm_year+1900, local.tm_mon + 1, local.tm_mday, local.tm_hour,local.tm_min,local.tm_sec);
  
  ESP_LOGD(TAG, "Uhrzeit: %s", systemCfg.last_payload);  
}
...
void time_init(void) {
#if (defined HAS_IF482 || defined HAS_DCF77)
  ESP_LOGI(TAG, "Starte clock Controller...");
  clock_init();
#endif
  ...
  ESP_LOGI(TAG, "Starte Zeitimpuls...");
