...
#error Doppelte Timesync-Methode ausgewählt. Sie müssen entweder LORASERVER oder LORAWAN Timesync auswählen.
#endif
...
void timesync_request(void) {
  ...
  // start timesync handshake
    ...
    ESP_LOGI(TAG, "[%0.3f] Timeserver Synchronisierungsanforderung gestartet, seqNo#%d",
...
// send timesync request
      ...
      if (xTaskNotifyWait(0x00, ULONG_MAX, &rcv_seqNo,
        ...
        ESP_LOGW(TAG, "[d%0.3f] Timesync abgebrochen: Zeitüberschreitung", _seconds());
        goto Fail; // no timestamp received before timeout
      }

      // check if we are in handshake with server
      if (rcv_seqNo != time_sync_seqNo) {
        ESP_LOGW(TAG, "[%0.3f] Timesync abgebrochen: Handshake nicht synchron",
                 _seconds());
        goto Fail;
      }
...
void timesync_serverAnswer(void *pUserData, int flag) {
  ...
  if (flag != TIME_SYNC_FRAME_LENGTH) {
    if (rcv_seqNo == TIME_SYNC_END_FLAG)
      ESP_LOGI(TAG, "[%0.3f] Timeserver error: keine sichere Zeit verfügbar",
               _seconds());
    else
      ESP_LOGW(TAG, "[%0.3f] Timeserver error: Falsche Daten empfangen",
               _seconds());
    goto Exit; // failure
  }
  ...
  if (flag != 1) {
    ESP_LOGW(TAG, "[%0.3f] Das Netzwerk hat die Zeitanfrage nicht beantwortet", _seconds());
    goto Exit;
  }
  ...
  if (flag != 1) {
    ESP_LOGW(TAG, "[%0.3f] Die Netzwerkzeitanforderung ist fehlgeschlagen", _seconds());
    goto Exit;
  }
...
Finish:
  ...
  if (timeIsValid(timestamp_sec)) {
    ...
    ESP_LOGW(TAG, "[%0.3f] Timeserver error: veraltete Uhrzeit erhalten",
             _seconds());
  }
