...
void handleSaveRestUrl(AsyncWebServerRequest *request)
{
    int params = request->params();
    ...
    for (int i = 0; i < params; i++)
    {
        ...
        if (p->isPost())
        {
            if (p->name() == "rname")
            {
                strcpy(newEntry.name, p->value().c_str());
            }
            else if (p->name() == "rkey")
            {
                strcpy(newEntry.api_key, p->value().c_str());
            }
            else if (p->name() == "rurl")
            {
                strcpy(newEntry.url, p->value().c_str());
            }
            else if (p->name() == "can_delete")
            {
                newEntry.can_delete = p->value() == "true" ? true : false;
            }
        }
    }

    doc["newid"] = insertRestUrl(newEntry);
    doc["status"] = 200;
    JsonArray rurls = doc.createNestedArray("list");
...
#pragma region Ajaxabfragen
    ...
    server.on(
        "/uplconfig", HTTP_POST, [](AsyncWebServerRequest *request)
        { request->send(200, "application/json", "{\"code\": 200}"); },
        handleConfigUpload);
    server.on("/saverurl", HTTP_POST, handleSaveRestUrl);
    ...
    AsyncCallbackJsonWebHandler *delete_resturl = new AsyncCallbackJsonWebHandler("/delrestentry", [](AsyncWebServerRequest *request, JsonVariant &json)
                                                                                  {
        ...
        deleteRestUrl(data["id"].as<uint16_t>());
        DynamicJsonDocument doc(2048);
        JsonArray rurls = doc.createNestedArray("rest_urls");
        for(resturls_t item: urlList) {
        JsonObject ud = rurls.createNestedObject();
        ...
        doc["status"] = 200;
        String njson = "";
        serializeJson(doc, njson);
        AsyncWebServerResponse *response = request->beginResponse(200, F(CONTENT_TYPE_JSON), njson);
        response->addHeader(F(CORS_HEADER), "*");
        request->send(response); });
    server.addHandler(check_wifi_handler);
    server.addHandler(delete_resturl);
#pragma endregion
